...
def f_noslip(g, m, R, I0, X):
    theta = X[0]
    dtheta = X[1]
    x = X[2]
    return np.array([dtheta, -m*g*R/I0*math.cos(theta), -R*dtheta*math.sin(theta)])
...
# Slip functions ###############################################################
def f_slip(g, m, R, Ig, uc, ii, X):
    theta = X[0]
    dtheta = X[1]
    x = X[2]
    dx = X[3]
    return np.array([dtheta, (dtheta**2*math.sin(theta)-g/R)*(math.cos(theta)-uc*math.sin(theta)*ii)/
                                (math.cos(theta)*(math.cos(theta)-uc*math.sin(theta)*ii)+Ig/(m*R**2)),
                    dx, uc*ii*m*(g+R*(ddtheta*math.cos(theta)-dtheta**2*math.sin(theta)))])

def ddtheta_slip(g, m, R, Ig, theta, dtheta, ii):
...
while y>0:
    ...
    if not slip:
        ...
        theta, dtheta, x = rk4([g, m, R, I0], dt, [theta, dtheta, x], f_noslip)
        ...
        theta, dtheta, x, dx = rk4([g, m, R, Ig, uc, ii], dt, [theta, dtheta, x, dx], f_slip)
        ddtheta = ddtheta_noslip(g, m, R, I0, theta)
